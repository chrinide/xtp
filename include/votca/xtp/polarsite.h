...
class PolarSite {
 ...
 public:
  ...
  PolarSite(const CheckpointReader& r) { ReadFromCpt(r); }

  PolarSite(const QMAtom& atom, double charge)
      : PolarSite(atom.getAtomID(), atom.getElement(), atom.getPos()) {
    setCharge(charge);
  }
  ...
  void setPolarisable(bool polarisable) { _isPolarisable = polarisable; }
  ...
  const Eigen::VectorXd& getPermMultipole() const {
    return _multipole;
  ...
  Eigen::Vector3d getField() const {
    return _localpermanetField + _localinducedField;
  }
