...
class Populationanalysis {
 public:
  void CalcChargeperAtom(Orbitals& orbitals, const AOBasis& basis,
    ...
    PolarSegment seg =
        PolarSegment(orbitals.QMAtoms().getName(), orbitals.QMAtoms().getId());
  ...
  template <typename S>
  Eigen::VectorXd CalcChargeperFragment(
      const std::vector<QMFragment<S> >& frags,
      const Eigen::VectorXd& atomcharges) {
