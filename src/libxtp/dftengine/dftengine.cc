...
bool DFTEngine::Evaluate() {
  ...
  for (int this_iter = 0; this_iter < _max_iter; this_iter++) {
    ...
    XTP_LOG(logDEBUG, *_pLog) << TimeStamp() << " DIIs error "
                              << _conv_accelerator.getDIIsError() << std::flush;

    XTP_LOG(logDEBUG, *_pLog) << TimeStamp() << " Delta Etot "
                              << _conv_accelerator.getDeltaE() << std::flush;
