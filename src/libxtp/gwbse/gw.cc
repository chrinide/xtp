    ...
    if ((i + _opt.qpmin) == _opt.homo) {
      XTP_LOG(logINFO, _log)
          << (boost::format("  HOMO  = %1$4d PQP = %2$+1.4f DQP = %3$+1.4f ") %
              (i + _opt.qpmin + 1) % _gwa_energies(i) % qp_diag_energies(i))
                 .str()
          << std::flush;
    } else if ((i + _opt.qpmin) == _opt.homo + 1) {
      XTP_LOG(logINFO, _log)
...
Eigen::VectorXd GW::ScissorShift_DFTlevel(
  ...
  RPAenergies.segment(_opt.homo + 1, dft_energies.size() - _opt.homo - 1)
      .array() += _opt.shift;
  return RPAenergies;
}
...
Eigen::VectorXd GW::CalculateExcitationFreq(const Eigen::VectorXd& rpa_energies,
                                            Eigen::VectorXd frequencies) {
  for (int i_freq = 0; i_freq < _opt.g_sc_max_iterations; ++i_freq) {

    _Sigma_c.diagonal() =
        _sigma->CalcCorrelationDiag(frequencies, rpa_energies);
...
void GW::CalculateGWPerturbation() {
  ...
  XTP_LOG(logDEBUG, _log) << TimeStamp() << " Prepared RPA  " << std::flush;
  for (int i_gw = 0; i_gw < _opt.gw_sc_max_iterations; ++i_gw) {
    ...
    rpa_energies =
        rpa.UpdateRPAInput(_dft_energies, frequencies, _opt.qpmin, _opt.homo);
...
void GW::CalculateHQP() {
  Eigen::VectorXd rpa_energies =
      RPA::UpdateRPAInput(_dft_energies, _gwa_energies, _opt.qpmin, _opt.homo);
