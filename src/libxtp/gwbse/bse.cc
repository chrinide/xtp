...
void BSE::SetupDirectInteractionOperator() {
  Eigen::VectorXd rpaenergies = RPA::UpdateRPAInput(
      _orbitals.MOEnergies(), _Hqp.diagonal(), _opt.qpmin, _opt.homo);
...
#pragma omp parallel for
  for (int c1 = 0; c1 < _bse_ctotal; c1++) {
    const MatrixXfd Mmn2T = factor * (_Mmn[c1 + _bse_cmin].block(
                                          _opt.vmin, 0, _bse_vtotal, auxsize) *
                                      _epsilon_0_inv.asDiagonal())
                                         .transpose();
    for (int v1 = 0; v1 < _bse_vtotal; v1++) {
      const MatrixXfd& Mmn1 = _Mmn[v1 + _opt.vmin];
      MatrixXfd Mmn1xMmn2T =
...
#pragma omp parallel for
  for (int v1 = 0; v1 < _bse_vtotal; v1++) {
    const MatrixXfd Mmn1 = factor * (_Mmn[v1 + _opt.vmin].block(
                                         _bse_cmin, 0, _bse_ctotal, auxsize))
                                        .transpose();
    for (int c1 = 0; c1 < _bse_ctotal; c1++) {
      int i1 = vc.I(v1, c1);
      for (int v2 = 0; v2 < _bse_vtotal; v2++) {
...
void BSE::Analyze_singlets(const AOBasis& dftbasis) {
  ...
  std::vector<Eigen::Vector3d> transition_dipoles =
...
BSE::Population BSE::FragmentPopulations(const QMStateType& type,
  ...
  XTP_LOG(logDEBUG, _log) << TimeStamp()
                          << " Filled DFT Overlap matrix of dimension: "
  ...
  Eigen::VectorXd pops = _orbitals.LoewdinPopulation(
      dmatgs, dftoverlap.Matrix(), dftbasis.getAOBasisFragA());
  ...
  for (int i_state = 0; i_state < _opt.nmax; i_state++) {
    ...
    Eigen::VectorXd popsH = _orbitals.LoewdinPopulation(
        dmat_ex[0], dftoverlap.Matrix(), dftbasis.getAOBasisFragA());
    pop.popH.push_back(popsH);
    // electron part
    Eigen::VectorXd popsE = _orbitals.LoewdinPopulation(
        dmat_ex[1], dftoverlap.Matrix(), dftbasis.getAOBasisFragA());
  ...
  XTP_LOG(logDEBUG, _log) << TimeStamp()
                          << " Ran Excitation fragment population analysis "
                          << flush;
...
std::vector<Eigen::Vector3d> BSE::CalcCoupledTransition_Dipoles(
    const AOBasis& dftbasis) {
  std::vector<Eigen::MatrixXd> interlevel_dipoles =
