...
void Density2Gyration::ReportAnalysis(
  ...
  XTP_LOG(logINFO, *_log) << "---------------- " << label << " ----------------"
                          << flush;
  XTP_LOG(logINFO, *_log) << (boost::format("  Norm               = %1$9.4f ") %
                              (gyro.mass))
                          << flush;
  ...
  XTP_LOG(logINFO, *_log) << (boost::format("  Tensor EF Axis 1 1 = %1$9.4f ") %
                              es.eigenvectors().col(0).x())
                          << flush;
  XTP_LOG(logINFO, *_log) << (boost::format("  Tensor EF Axis 1 2 = %1$9.4f ") %
                              es.eigenvectors().col(0).y())
                          << flush;
  XTP_LOG(logINFO, *_log) << (boost::format("  Tensor EF Axis 1 3 = %1$9.4f ") %
                              es.eigenvectors().col(0).z())
                          << flush;
  XTP_LOG(logINFO, *_log) << (boost::format("  Tensor EF Axis 2 1 = %1$9.4f ") %
                              es.eigenvectors().col(1).x())
                          << flush;
  XTP_LOG(logINFO, *_log) << (boost::format("  Tensor EF Axis 2 2 = %1$9.4f ") %
                              es.eigenvectors().col(1).y())
                          << flush;
  XTP_LOG(logINFO, *_log) << (boost::format("  Tensor EF Axis 2 3 = %1$9.4f ") %
                              es.eigenvectors().col(1).z())
                          << flush;
  XTP_LOG(logINFO, *_log) << (boost::format("  Tensor EF Axis 3 1 = %1$9.4f ") %
                              es.eigenvectors().col(2).x())
                          << flush;
  XTP_LOG(logINFO, *_log) << (boost::format("  Tensor EF Axis 3 2 = %1$9.4f ") %
                              es.eigenvectors().col(2).y())
                          << flush;
  XTP_LOG(logINFO, *_log) << (boost::format("  Tensor EF Axis 3 3 = %1$9.4f ") %
