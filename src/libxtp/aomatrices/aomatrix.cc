...
template <class T>
void AOMatrix<T>::Print(std::string ident) {
  std::cout << "\n" << std::endl;
  std::cout.precision(12);
  for (int i = 0; i < _aomatrix.rows(); i++) {
    for (int j = 0; j < _aomatrix.cols(); j++) {
      std::cout << ident << "[" << i + 1 << ":" << j + 1 << "] "
                << std::scientific << _aomatrix(i, j) << std::endl;
    }
  }
}
...
void AOMatrix3D::Print(std::string ident) {
  ...
  for (int i = 0; i < _aomatrix[0].rows(); i++) {
    for (int j = 0; j < _aomatrix[0].cols(); j++) {
      std::cout << ident << "[" << i + 1 << ":" << j + 1 << "] "
                << _aomatrix[0](i, j) << " : " << _aomatrix[1](i, j) << " : "
                << _aomatrix[2](i, j) << std::endl;
    }
  }
}
