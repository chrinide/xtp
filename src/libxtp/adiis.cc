...
Eigen::VectorXd ADIIS::CalcCoeff(
    const std::vector<std::unique_ptr<Eigen::MatrixXd> >& dmathist,
    const std::vector<std::unique_ptr<Eigen::MatrixXd> >& mathist) {
  success = true;
  ...
  for (int i = 0; i < size; i++) {
    for (int j = 0; j < size; j++) {
      DiFj(i, j) =
          ((*dmathist[i]) - dmat).cwiseProduct((*mathist[j]) - H).sum();
    }
  }
