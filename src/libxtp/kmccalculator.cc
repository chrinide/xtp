...
void KMCCalculator::LoadGraph(Topology* top) {
  ...
  for (const QMPair* pair : nblist) {
    _nodes[pair->Seg1()->getId() - 1].AddEventfromQmPair(
        *pair, _carriertype.ToSegIndex(), _nodes);
    _nodes[pair->Seg2()->getId() - 1].AddEventfromQmPair(
        *pair, _carriertype.ToSegIndex(), _nodes);
  }
...
void KMCCalculator::InitialRates() {
  ...
  for (auto& node : _nodes) {
    for (auto& event : node.events) {
      ...
      double reorg = node.reorg_intorig + event.destination->reorg_intdest +
                     event.reorg_out;
