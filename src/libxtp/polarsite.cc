...
PolarSite::PolarSite(int id, std::string element, Eigen::Vector3d pos)
    : _id(id),
      ...
      _pos(pos),
      _rank(0),
      _multipole(Eigen::VectorXd::Zero(1)),
      _isPolarisable(false),
...
void PolarSite::calcRank() {
  // Get Rank: We have at the moment just three cases. Point charges,dipoles and
  // Quadrupoles
...
Eigen::VectorXd PolarSite::CalculateSphericalMultipole(
...
Eigen::MatrixXd PolarSite::FillTholeInteraction(const PolarSite& otherSite,
                                                double a) {
  ...
  const double fac2 = std::pow(fac0, 3);
  const double au3 =
      a / (fac2 * std::sqrt(this->_eigendamp * otherSite._eigendamp));
  double lambda3 = 1.0;
  double lambda5 = 1.0;
  double lambda7 = 1.0;
  if (au3 < 40) {
...
double PolarSite::InteractStatic(PolarSite& otherSite) {
  ...
  if (this->isPolarisable()) {
    this->_localpermanetField +=
        (Tab.block(1, 0, 3, sizeB) * multipolesB).rowwise().sum();
  }
  if (otherSite.isPolarisable()) {
    otherSite._localpermanetField +=
        (multipolesA.transpose() * Tab.block(0, 1, sizeA, 3))
            .colwise()
            .sum()
            .transpose();
  }
  // Energy
  const double EnergyAB =
      multipolesA.transpose() * Tab.topLeftCorner(sizeA, sizeB) *
...
double PolarSite::InteractInduction(PolarSite& otherSite, double a) {
  ...
  const Eigen::VectorXd& multipolesB = otherSite.getPermMultipole();  // Q^(b)_u
  double EnergyAB = 0;
  const Eigen::MatrixXd tTab =
      FillTholeInteraction(otherSite, a);  // \tilde{T}^(ab)_tu
  // Calculate Potential due to induced Dipoles
  if (otherSite.isPolarisable()) {
    this->PhiU += (tTab.row(0).segment<3>(1)) * otherSite._inducedDipole;
    EnergyAB += multipolesA.transpose() *
                tTab.block(0, 1, multipolesA.size(), 3) *
                otherSite._inducedDipole;
  }
  if (this->isPolarisable()) {
    otherSite.PhiP +=
        (this->_inducedDipole.transpose() * tTab.col(0).segment<3>(1)).sum();
    EnergyAB += this->_inducedDipole.transpose() *
                tTab.block(1, 0, 3, multipolesB.size()) * multipolesB;
  ...
  if (this->isPolarisable() && otherSite.isPolarisable()) {
    ...
    this->_localinducedField +=
        (Tdd * otherSite._inducedDipole).rowwise().sum();
    otherSite._localinducedField +=
        (this->_inducedDipole.transpose() * Tdd).colwise().sum().transpose();
    // Calculate Interaction induced Dipole induced Dipole
    EnergyAB +=
        this->_inducedDipole.transpose() * Tdd * otherSite._inducedDipole;
  }
  return EnergyAB;
}
...
std::string PolarSite::WriteMpsLine(string unit) const {
  ...
  if (unit == "angstrom") {
    ...
    throw std::runtime_error(
        " PolarSite::WriteMpsLine: Unit conversion not known");
  }
  std::string output = "";
  ...
  output +=
      (boost::format(
           "     P %1$+1.7f %2$+1.7f %3$+1.7f %4$+1.7f %5$+1.7f %6$+1.7f\n") %
